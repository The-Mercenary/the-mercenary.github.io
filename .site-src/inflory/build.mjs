...
function page(key, isRoot = false) {
  const w = waitlist[key];
  const t = {...locales[key], nav:[...locales[key].nav.slice(0,2),w.cta], closingCta:w.cta, closingNote:w.ctaNote,
    planCta:w.ctaPlan, noScript:w.noScript, privacyTitle:w.privacyTitle,
    faq:locales[key].faq.map((entry,i)=>i===0?[entry[0],w.faq]:entry),
...
<section class="plans-section wrap" id="plans" aria-labelledby="plans-title"><div class="section-head"><div><p class="eyebrow">FIND YOUR RHYTHM</p><h2 id="plans-title">${lines(t.plansTitle)}</h2></div><p class="section-note">${lines(t.plansNote)}</p></div><div class="plans-grid">${plans.map(([id,p],i)=>`<article class="plan ${i===1?'plan-featured':''}" data-plan-card="${id}"><div class="plan-top"><span class="plan-index">0${i+1}</span><span class="plan-caption">${escape(t.captions[i])}</span></div><h3>${p.name}</h3><p class="plan-summary">${escape(t.summaries[i])}</p><p class="plan-quantity">${p.followers}<span>${escape(t.quantitySuffix)}</span></p><p class="plan-price"><strong>$${p.price}</strong> USD <span>${escape(t.monthSuffix)}</span></p><div class="plan-divider"></div><ul>${[t.benefits[0],t.benefits[1],t.benefits[i===0?2:3]].map(x=>`<li>${escape(x)}</li>`).join('')}</ul><a class="button ${i===1?'button-cream':'button-outline'}" data-apply data-plan="${id}" data-placement="plan" href="/inflory/${key}/apply/?plan=${id}">${escape(format(t.planCta,{plan:p.name}))} ${arrow}</a></article>`).join('')}</div><p class="pricing-note">${escape(t.pricingNote)}</p></section>
...
<section class="closing-section"><div class="wrap closing-inner"><div><p class="eyebrow">YOUR NEXT MONTH, IN MOTION</p><h2>${lines(t.closing)}</h2></div><div class="closing-action"><a class="button button-cream" data-apply data-placement="footer" href="/inflory/${key}/apply/">${escape(t.closingCta)} ${arrow}</a><p>${escape(t.closingNote)}</p></div></div></section><div class="disclosure wrap"><p>${escape(t.disclosure)}</p></div></main>
